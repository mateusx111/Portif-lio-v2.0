...
const Nav = () => {
  ...
  return (
    ...
    z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-md xl:h-screen"
    >
      {/*inner*/}
      <div
        className="flex w-full xl:flex-col items-center justify-between xl:justify-center gap-y-10 px-4 md:px-40
    xl:px-0 h-[80px] xl:h-max py-8 bg-white/10 backdrop-blur-sm text-3xl xl:text-2xl xl:rounded-full"
      >
        {navData.map((link, index) => {
          return (
            <Link
              className={`${
                link.path === pathname && "text-orange-400"
              } relative flex items-center group hover:text-orange-500 transition-all duration-300`}
              href={link.path}
              key={index}
            >
              {/* tooltip */}
